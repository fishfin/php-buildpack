    ...
    def install_extensions(self):
        ...
        for ibmdbExtn in ['IBM_DB2']: #, 'PDO', 'PDO_IBM']:
           ...
           #              self._phpExtnDir])
            ...
            time.sleep(5)
            subprocess.call(['ls', '-l'])
            self._runCmd(osev,extnDownloadDir, ['php', '-i'])
            self._runCmd(osev,extnDownloadDir, ['phpize'])
            #self._runCmd(osev,extnDownloadDir, ['pecl','install','ibm_db2'])
            self._runCmd(osev, extnDownloadDir,['./configure -with-IBM_DB2='+ self._ctx['IBMDBCLIDRIVER_INSTALL_DIR']] )
